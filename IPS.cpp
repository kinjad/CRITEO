...
void computeGradient(vector<double> w, double &x, vector<double> &g, vector<vector<vector<int> > > data_idx, vector<vector<vector<double> > > data_val, vector<double> data_reward, vector<double> data_ips, int start, int end){
...
//		cout << pos_freq[i] << " " << neg_freq[i] << endl;
	...
	for(int t = 0; t < num_iter; t++){
		...
		computeGradient(w, x_test, g, data_idx, data_val, data_reward, data_ips, total_num/3*2, total_num);

		computeGradient(w, x_train, g, data_idx, data_val, data_reward, data_ips, 0, total_num/3*2);
...
//			if(w[i] > 0) w[i] = 0;
		...
		cout << "Iter " << t << ": Train Obj value = " << x_train*10000 << " Test Obj value = " << x_test*10000 << endl;
	}
